<h2>Просто курсовая работа</h2>
Морской бой. <br>
Особенности: наличие GUI. <br>
Использованный язык: C++. <br>
Использованный стандарт: C++20. <br>
Использованные библиотеки: SFML, ImGui, ImGui-sfml. <br>
Система сборки: cmake. <br>

<h2>Как оно работает?</h2>

![Class Diagram](http://www.plantuml.com/plantuml/proxy?src=https://raw.githubusercontent.com/Fluiser/courseproject/refs/heads/main/howto.uml)
